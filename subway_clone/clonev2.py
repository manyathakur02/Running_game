"""
Subway Surfers-style endless runner (Step 2 of the build plan - 2D Obstacles Edition).
...
OBSTACLE_MIN_GAP = 480    # min vertical px between spawned obstacles
...
class Obstacle:
    ...
    def draw(self, surf):
        # Flattened 2D rendering back to single-color rects
...
class Coin:
    ...
    def draw(self, surf):
        if not self.collected:
            # Reverted back to flat 2D coin representation
...
class Game:
    def __init__(self, input_provider: InputProvider):
        ...
        pygame.display.set_caption("Subway Clone -- Flat 2D Build")
    ...
    def spawn_row(self):
        ...
        # Keeps the multi-coin stream functionality 
        coin_count = random.randint(3, 6)
        coin_lane = random.choice([l for l in range(LANE_COUNT) if l != lane])
        
        for i in range(coin_count):
            offset_y = spawn_y - (i * 65)
            self.coins.append(Coin(coin_lane, offset_y))
    ...
    def draw(self):
        ...
        self.screen.blit(coin_surf, (12, 44))
        ...
        self.screen.blit(intensity_surf, (12, 76))
